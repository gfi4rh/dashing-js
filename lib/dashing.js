var fs = require('fs'),
    ...
    momentNode = require('moment');
momentNode.locale('fr');
...
module.exports.Dashing = function Dashing() {
    ...
    dashing.default_dashboard = 'dashboard-gfi4rh';
    dashing.port = (process.env.PORT || 8080);
    ...
    var auth = function (req, res, next) {
        ...
        if (user.name === 'admin' && user.pass === 'Gfi4Rh!') {
            return next();
        } else {
            return unauthorized(res);
        }
    ...
    app.get('/dashboard-gfi4rh/send-message', auth, function (req, res) {
        var dashboard = "dashboard-gfi4rh/send-message";
    ...
    app.get('/dashboard-gfi4rh/jira', function (req, res) {
        var dashboard = "dashboard-gfi4rh/jira";
        fs.exists([dashing.views, dashboard + '.' + dashing.view_engine].join(path.sep), function (exists) {
            if (exists) {
                res.render(dashboard, {
                    dashboard: dashboard,
    ...
    app.get('/events', dashing._protected, events());

    app.get('/dashboard-gfi4rh/events', dashing._protected, events());
    ...
    app.post('/widgets/auth', function (req, res) {
        var auth_token = req.body.auth_token;
        if (!dashing.auth_token || dashing.auth_token === auth_token) {
            if (req.body.user === '21232f297a57a5a743894a0e4a801fc3' && req.body.password === '9253fadb0643b06b2276524b75e1d5d1') {
                res.send('OK');
            } else {
                res.send('Mauvaise saisie');
            }
        } else {
    ...
    function send_event(id, body) {
        ...
        body.updatedAtFrench = momentNode().format('L') + " à " + momentNode().format('LT');
